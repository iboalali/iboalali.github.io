...
- **Signal types tracked:** `pageview` (Web SDK auto-pageview), plus the four custom signals below
- **Custom signal:** `App.Referral` (fires only when `utm_source=android_app`)
- **Custom signal:** `Theme.Toggle` (fires once per burst of toggle clicks, debounced ~1.5s; payload `Theme.Toggle.from` is the mode the burst started from and `Theme.Toggle.to` is the resting mode they settled on — `light`, `dark`, `paper`, or `auto`. Bursts that cycle back to the starting mode emit nothing)
- **Custom signal:** `Contact.Click` (fires when a visitor opens one of the contact-page social cards; payload `Contact.Click.platform` names the platform — `Mastodon`, `Github`, `LinkedIn`, or `Bluesky`)
- **Custom signal:** `Shortlink.Visit` (fires once when a visitor arrives through a short link; payload `Shortlink.Visit.slug` is the alias used — `bb`, `brc`, or `hpn` — and `Shortlink.Visit.landing` is the path they landed on)

> **History:** the four custom signals above were dead code until 2026-08-14. The
> site loaded only the TelemetryDeck **Web** SDK, which sends the auto-pageview
> and exposes no API at all, so `window.td` was never defined and every
> `td.signal()` call silently no-opped behind its own guard. That is why earlier
> versions of this file described them as "planned, not yet present in the
> structural data export" — they had never fired. Adding the vendored
> TelemetryDeck **JavaScript** SDK (which is the one with `signal()`) alongside
> the Web SDK fixed all four at once. See the Analytics notes in `CLAUDE.md`.

## Seeing custom signals in the dashboard

1. **Local development is test data.** Signals from `localhost` are sent with
   `isTestMode: true` (the Web SDK detects localhost itself; the JavaScript SDK is
   passed the flag explicitly in `main_layout.njk`). They appear only when **Test
   Mode** is switched on in the dashboard, so local clicking never pollutes live
   numbers. This is the fastest way to confirm a new signal works: run the dev
   server, trigger it, flip Test Mode on.
2. **Production needs a deploy.** Push to `main`, wait for the Actions build, then
   trigger the signal on the live site.
3. **Custom payload keys only become selectable after data arrives.** A dimension
   like `Shortlink.Visit.slug` will not appear in the dashboard's dropdowns until
   at least one signal carrying it has been ingested. Pasting the custom queries
   below works regardless, but returns empty until then.
4. **Re-export structural data** from the dashboard afterwards to refresh
   `docs/iboalali-com-StructuralData.json`, which is still a pageview-only export.
...
## Once `Shortlink.Visit` starts arriving

`Shortlink.Visit` fires from `main.js` once per arrival through one of the short
links (`/bb`, `/brc`, `/hpn`). The stubs generated by `shortlinks.11ty.js`
redirect within milliseconds, too fast for the deferred SDK to send from the stub
itself, so each stub stashes its slug in `sessionStorage` and the destination page
fires the signal. `Shortlink.Visit.slug` is the alias used and
`Shortlink.Visit.landing` is the path it resolved to.

Because the key is cleared on read, this counts *arrivals*, not pageviews: one
signal per visitor per trip through a short link, however many pages they then
browse. That makes `eventCount` the right aggregation for "how many times was the
QR code scanned" and `userCount` for "how many distinct people".

Note this is independent of `utm_*`. A short link that also carries a tag (say
`iboalali.com/hpn?utm_source=qr`) fires `Shortlink.Visit` *and* keeps its UTM
parameters, so it shows up in the UTM queries above too. Assumes the signal type
and payload keys have appeared in the structural data export.

### 13. Most-used short links

Ranks the aliases by number of arrivals.
...
```json
{
  ...
  "granularity": "all",
  "threshold": 10,
  "dimension": { "type": "default", "dimension": "Shortlink.Visit.slug", "outputName": "Short link" },
  "metric": { "type": "numeric", "metric": "arrivals" },
  "aggregations": [{ "type": "eventCount", "name": "arrivals" }],
  "filter": {
    "type": "and",
    "fields": [
      { "type": "selector", "dimension": "type", "value": "Shortlink.Visit" },
      { "type": "selector", "dimension": "isBot", "value": "False" }
    ]
  },
  "baseFilters": "thisApp",
...
### 14. Short-link arrivals over time

Daily arrivals and distinct visitors per alias. Group by day, then sort the table
by clicking a column header (`limitSpec` is dropped by the editor, see the quirks
section above).
...
```json
{
  ...
  "granularity": "day",
  "dimensions": [
    { "type": "default", "dimension": "Shortlink.Visit.slug", "outputName": "Short link" }
  ],
  "aggregations": [
    { "type": "eventCount", "name": "arrivals" },
    { "type": "userCount", "name": "visitors" }
  ],
  "filter": {
    "type": "and",
    "fields": [
